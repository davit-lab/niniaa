...
function ReviewsAdmin() {
  ...
  const { data: reviews = [] } = useQuery({
    ...
    queryFn: async () => {
      const q = query(collection(db, "reviews"), orderBy("sort_order"));
  ...
  async function save() {
    ...
    try {
      if (isNew) {
        await addDoc(collection(db, "reviews"), { ...payload, created_at: serverTimestamp() });
      } else {
        await updateDoc(doc(db, "reviews", editing.id!), payload);
      }
